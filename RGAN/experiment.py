...
for epoch in range(num_epochs):
    ...
    if epoch % eval_freq < 0: #wzh: change it to be false to avoid additional memory usage.
    ...
    try:
        ...
        print('%d\t%.2f\t%.4f\t%.4f\t%s\t%s\t %s\t %s' % (epoch, t, D_loss_curr, G_loss_curr, mmd2, that_np, pdf_sample, pdf_real)) #wzh: change some output into string format.
