...
def get_data(data_type, data_options=None):
    ...
    elif data_type == 'energy': #wzh: new energy dataset
        samples = energy_load(**data_options)
    elif data_type == 'stock': #wzh: new stock dataset
        samples = stock_load(**data_options)
    elif data_type == 'sines': #wzh: new sines dataset
        # samples = sines_load(**data_options)
        src_path = "./experiments/data/"
        file_name = "sines_timeGAN.npy"
        samples = np.load(os.path.join(src_path, file_name))
    else:
...
#wzh: load energy dataset
def energy_load(seq_length=24, **kwargs):
    """ Load the new Energy dataset """
    try:
        train = np.load('./experiments/data/energy_train.npy')
        print('Loaded energy from .npy')
    except IOError:
        print('Failed to load Energy data from .npy, loading from csv')
        train = np.loadtxt(open('./experiments/data/energy_data.csv', 'r'), delimiter=',', skiprows = 1)
        np.save('./experiments/data/energy_train.npy', train)
    ...
    temp_data = []
    seq_len = seq_length
    for i in range(0, len(train) - seq_len):
        _x = train[i:i + seq_len]
        temp_data.append(_x)
    idx = np.random.permutation(len(temp_data))
    samples = []
    for i in range(len(temp_data)):
        samples.append(temp_data[idx[i]])
    samples = np.array(samples)
    
    ### =========== Get only 10% data ============= ###
    num_samples = len(samples)
    p_rate = int(np.ceil(0.1 * num_samples)) # 10% of original samples
    new_samples = samples[:p_rate, :, :] #(367, 24, 6)
    print("new 10% data shape: ", new_samples.shape)
    samples = new_samples
    ### =========================================== ###
    return samples

def stock_load(seq_length=24, **kwargs):
    """ Load the new Stock dataset """
    try:
        train = np.load('./experiments/data/stock_train.npy')
        print('Loaded stock from .npy')
    except IOError:
        print('Failed to load Stock data from .npy, loading from csv')
        train = np.loadtxt(open('./experiments/data/stock_data.csv', 'r'), delimiter=',', skiprows = 1)
        np.save('./experiments/data/stock_train.npy', train)
