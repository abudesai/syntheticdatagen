...
class Generator(nn.Module):
    ''' C-RNN-GAN generator
    '''
    def __init__(self, num_feats, hidden_units=64, drop_prob=0.0, use_cuda=False):
    ...
    def forward(self, z, states):
        ...
        gen_feats = torch.sigmoid(gen_feats)
...
class Discriminator(nn.Module):
    ...
    def forward(self, note_seq, state):
        ...
        # mini = torch.amin(out, dim=reduction_dims)        
        # maxi = torch.amax(out, dim=reduction_dims)        
        # out = maxi - mini


        # out = torch.amax(out, dim=reduction_dims)        
