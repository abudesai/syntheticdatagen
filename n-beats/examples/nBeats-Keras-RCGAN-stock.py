...
#syn_data = syn_data[:,:,:6] #cut and only use the first 6 columns
...
backend.compile(loss='mae', optimizer=opt)
...
for idx in range(num_times):
  mdname = 'nBeats-RCGAN-stock-best.h5'
  ...
  logname = 'Log-' + "nBeats-RCGAN-stock"  + '-.txt'
  logfile = os.path.join(logpath, logname)
  csv_logger = CSVLogger(logfile, append=True)
  reduceLR = ReduceLROnPlateau(monitor='val_loss', factor=0.05, patience=10)
  ...
  backend.save('n_beats_model_rcgan_stock.h5')
  ...
  modelname = os.path.join(src_path, "nBeats-RCGAN-stock-best.h5")
...
print('std of Predictive MAE score: ' + str(np.round(np.std(num_MAE), 8)))
