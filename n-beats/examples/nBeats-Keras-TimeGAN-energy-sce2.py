...
'''
# it is not proper for the Scenario-2 & 3        
...
def real_data_loading_sce2(data_name, seq_len, test_per, observe_per):
  ...
  #New normalization - will be normalized later
  #norm_constant = np.max(train_data)
  #train_data = train_data / norm_constant
  #test_data = test_data / norm_constant
...
# load the generated data -- TimeGAN's data
src_path = "/data/home/stufs1/zuwang/dg21/TimeGAN/"
filename = "energy_data_TimeGAN_5000_sce2-wTest-0-010.npy"

seq_len = 24
syn_data = np.load(os.path.join(src_path, filename))

# additional normalization for synthetic data
...
num_samples, time_steps, input_dim, output_dim = 1048551, 24, 28, 28
backend = NBeatsKeras(
        input_dim=input_dim,
...
for idx in range(num_times):
  mdname = 'nBeats-TimeGAN-energy-best-wTest-0-010.h5'
  ...
  logname = 'Log-' + "nBeats-TimeGAN-energy-wTest-0-010"  + '-.txt'
  ...
  #backend.save('n_beats_model_abu_energy.h5')
  ...
  modelname = os.path.join(src_path, "nBeats-TimeGAN-energy-best-wTest-0-010.h5")
...
plt.savefig("nbeats-predictions-energy.png", dpi=300)
plt.show()
